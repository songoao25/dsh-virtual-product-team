...
- For supplementary content near a conversation, first inspect the current alpha.4 public slot catalog and choose a supported slot. Use `conversation.session.header.utilities` for session-level utility or status content, `conversation.view` for a conversation surface, or the tool-view slots for tool-specific content. Do not use the removed conversation chat-tail slot or invent a slot name.
